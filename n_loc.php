...
$_SESSION['datel'] = $_POST['datel'];
$_SESSION['dater'] = $_POST['dater'];
$_SESSION['datef'] = date("d-m-Y");
$_SESSION['id_cha'] = $_POST['chauffeur'];
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="#">Informations du client</a>
	...
	<?php	
		if($_POST['chauffeur'] =="client")
			...
			else
			{
						...
						$reqr = $bdd->query('SELECT * FROM chauffeur where id='.$_POST['chauffeur'].' ;');
						$chaud = $reqr->fetch();
						$reqre = $bdd->query('SELECT * FROM reservations ;');						
			while ($datae= $reqre->fetch())
			{	
					if($datae['id_cha']==$chaud['id'])			
												
				echo' <div>
					<div  class="row-fluid sortable mesdiv">
					...
					<div class="box-content">
						<form class="form-horizontal" method="post" action="setting.php" enctype = "multipart/form-data">
						  <fieldset>
							...
							<strong><h2><i class="icon-warning-sign"></i> Attention!</h2></strong> <h1>il y a une réservation prévue du '.date("d-m-Y", strtotime($datae['datel'])).' au '.date("d-m-Y", strtotime($datae['dater'])).' pour le chauffeur '.$chaud['nom'].' '.$chaud['prenom'].' <br> assurez vous d\'avoir choisit une date différente </h1>
						</div>
	...
	?>		
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					...
					<div class="box-content">
						<form class="form-horizontal" method="post" action="factu.php"  enctype = "multipart/form-data">
						  <fieldset>
							<div class="control-group">
							  ...
							  <div class="control-group success">
								<label class="control-label" for="inputSuccess">Numéro de permis</label>
								<div class="controls">
								  <input type="text" id="inputSuccess" name="numPermis">
								  <span class="help-inline"></span>
								</div>
							  </div>
							  <div class="control-group  success">
									...
									<?php						
								$reqe = $bdd->query('SELECT * FROM permis ;');
								while ($datae= $reqe->fetch())
								{	
									
									echo '<option value="'.$datae['nom'].'">'.$datae['nom'].'</option>';							
									}	
						  ...
						  <fieldset>
							<div class="control-group">
							 ...
							 <div class="alert alert-success">	
							  ...
							  <div class="control-group  success">
															<label class="control-label" for="selectError">Nom ou Prénom</label>
															<div class="controls">
															  <select name="ancienClient" data-rel="chosen">														
									<?php						
								$reqe = $bdd->query('SELECT * FROM client ;');
								while ($datae= $reqe->fetch())
								{	
									
									echo '<option value="'.$datae['id'].'">'.$datae['nom'].' '.$datae['prenom'].'</option>';							
									}	
